                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    ...
                                    <div class="card-body">
                                        ...
                                        <form action="{{ route('manage-infomation.update', $infomationS->id) }}"
                                            method="post" enctype="multipart/form-data">
                                            @csrf
                                            @method('PUT')
                                            <div class="form-group">
                                                <label>หัวข้อข่าว</label>
                                                <input type="name" name="info_title"
                                                    value="{{ $infomationS->info_title }}" class="form-control"
                                                    placeholder="" {{ $type === 'edit' ? 'required' : 'readonly' }}>
                                            </div>
                                            <div class="form-group">
                                                <label>โรงงาน</label>
                                                <select class="form-control select2" name="info_factory"
                                                    style="width: 100%;" {{ $type === 'edit' ? 'required' : 'disabled' }}>
                                                    <option value="{{ $infomationS->info_factory }}">
                                                        {{ $infomationS->fac_category }}</option>
                                                    @foreach ($factory as $factoryS)
                                                        <option value="{{ $factoryS->id }}">{{ $factoryS->fac_name }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                            </div>
